...
import { PaperLabelDto } from '../../types'
...
const emit = defineEmits<{
  (event: 'edit', label: PaperLabelDto): void
  (event: 'delete', label: PaperLabelDto): void
  (event: 'selectedLabel', label: PaperLabelDto): void
}>()

const props = defineProps({
  labels: {
    type: Array as PropType<PaperLabelDto[]>,
    required: true,
  },
  loading: {
    type: Boolean,
...
const handleSelectionChange = (selectedItems: PaperLabelDto[]) => {
...
<template>
  ...
  >
    ...
    <template #cell(actions)="{ rowData: label }">
      <div class="flex gap-2 justify-end">
        <VaButton
          ...
          @click="emit('edit', label as PaperLabelDto)"
        />
        <VaButton
          preset="primary"
          size="small"
          ...
          @click="emit('delete', label as PaperLabelDto)"
        />
      </div>
    </template>
  </VaDataTable>
