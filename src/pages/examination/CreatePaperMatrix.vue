...
import { ContentMatrixRequest, CreateMatrixRequest } from './types'
...
const continueCreatePaper = () => {
  paperMatrixStore
    ...
    .then(() => notify({ message: 'thanh cong', color: 'success' }))
    .catch((error) => notify({ message: `that bai ${error}`, color: 'danger' }))
}
...
<template>
  <VaCard class="mb-2">
    <VaCardContent class="grid grid-cols-12 gap-2">
      ...
      />
      <VaSelect placeholder="created matrix list" clearable class="md:col-span-2 xs:col-span-12" />
  ...
  <VaCard>
    ...
    <VaCardContent>
      <p class="text-secondary">
        *Raw index: Enter the positions in the title: The positions are separated by commas ',' (eg. To get 3 sentences,
        enter: 1, 2, 3). If you don't want to sort, you can leave it blank
      </p>
